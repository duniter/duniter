...
const UDID2 =
  "udid2;c;([A-Z-]*);([A-Z-]*);(\\d{4}-\\d{2}-\\d{2});(e\\+\\d{2}\\.\\d{2}(\\+|-)\\d{3}\\.\\d{2});(\\d+)(;?)";
...
module.exports = {
  ...
  ERRORS: {
    ...
    UNKNOWN: {
      httpCode: 500,
      uerr: { ucode: 1001, message: "An unknown error occured" },
    },
    UNHANDLED: {
      httpCode: 500,
      uerr: { ucode: 1002, message: "An unhandled error occured" },
    },
    SIGNATURE_DOES_NOT_MATCH: {
      httpCode: 400,
      uerr: { ucode: 1003, message: "Signature does not match" },
    },
    ALREADY_UP_TO_DATE: {
      httpCode: 400,
      uerr: { ucode: 1004, message: "Already up-to-date" },
    },
    ...
    SANDBOX_FOR_IDENTITY_IS_FULL: {
      ...
      uerr: {
        ucode: 1007,
        message:
          "The identities' sandbox is full. Please retry with another document or retry later.",
      },
    },
    SANDBOX_FOR_CERT_IS_FULL: {
      ...
      uerr: {
        ucode: 1008,
        message:
          "The certifications' sandbox is full. Please retry with another document or retry later.",
      },
    },
    SANDBOX_FOR_MEMERSHIP_IS_FULL: {
      httpCode: 503,
      uerr: {
        ucode: 1009,
        message:
          "The memberships' sandbox is full. Please retry with another document or retry later.",
      },
    },
    SANDBOX_FOR_TRANSACTION_IS_FULL: {
      httpCode: 503,
      uerr: {
        ucode: 1010,
        message:
          "The transactions' sandbox is full. Please retry with another document or retry later.",
      },
    },
    NO_POTENTIAL_FORK_AS_NEXT: {
      httpCode: 503,
      uerr: {
        ucode: 1011,
        message:
          "No fork block exists in the database as a potential next block.",
      },
    },
    INCONSISTENT_DB_MULTI_TXS_SAME_HASH: {
      httpCode: 503,
      uerr: {
        ucode: 1012,
        message: "Several transactions written with the same hash.",
      },
    },
    CLI_CALLERR_RESET: {
      httpCode: 503,
      uerr: {
        ucode: 1013,
        message:
          "Bad command: usage is `reset config`, `reset data`, `reset peers`, `reset stats` or `reset all`",
      },
    },
    CLI_CALLERR_CONFIG: {
      httpCode: 503,
      uerr: { ucode: 1014, message: "Bad command: usage is `config`." },
    },
    CLI_CALLERR_WS2P: {
      httpCode: 503,
      uerr: { ucode: 1014, message: "Bad command: usage is `ws2p [subcmd]`." },
    },

    // Business errors
    NO_MATCHING_IDENTITY: {
      httpCode: 404,
      uerr: { ucode: 2001, message: "No matching identity" },
    },
    UID_ALREADY_USED: {
      httpCode: 400,
      uerr: { ucode: 2002, message: "UID already used in the blockchain" },
    },
    PUBKEY_ALREADY_USED: {
      httpCode: 400,
      uerr: { ucode: 2003, message: "Pubkey already used in the blockchain" },
    },
    NO_MEMBER_MATCHING_PUB_OR_UID: {
      httpCode: 404,
      uerr: { ucode: 2004, message: "No member matching this pubkey or uid" },
    },
    WRONG_SIGNATURE_MEMBERSHIP: {
      httpCode: 400,
      uerr: { ucode: 2006, message: "wrong signature for membership" },
    },
    ALREADY_RECEIVED_MEMBERSHIP: {
      httpCode: 400,
      uerr: { ucode: 2007, message: "Already received membership" },
    },
    MEMBERSHIP_A_NON_MEMBER_CANNOT_LEAVE: {
      httpCode: 400,
      uerr: { ucode: 2008, message: "A non-member cannot leave" },
    },
    NOT_A_MEMBER: {
      httpCode: 400,
      uerr: { ucode: 2009, message: "Not a member" },
    },
    BLOCK_NOT_FOUND: {
      httpCode: 404,
      uerr: { ucode: 2011, message: "Block not found" },
    },
    ...
    NO_IDTY_MATCHING_PUB_OR_UID: {
      httpCode: 404,
      uerr: { ucode: 2021, message: "No identity matching this pubkey or uid" },
    },
    NEWER_PEER_DOCUMENT_AVAILABLE:
      CommonConstants.ERRORS.NEWER_PEER_DOCUMENT_AVAILABLE,
    PEER_DOCUMENT_ALREADY_KNOWN:
      CommonConstants.ERRORS.PEER_DOCUMENT_ALREADY_KNOWN,
    TX_INPUTS_OUTPUTS_NOT_EQUAL:
      CommonConstants.ERRORS.TX_INPUTS_OUTPUTS_NOT_EQUAL,
    TX_OUTPUT_SUM_NOT_EQUALS_PREV_DELTAS:
      CommonConstants.ERRORS.TX_OUTPUT_SUM_NOT_EQUALS_PREV_DELTAS,
    BLOCKSTAMP_DOES_NOT_MATCH_A_BLOCK:
      CommonConstants.ERRORS.BLOCKSTAMP_DOES_NOT_MATCH_A_BLOCK,
    A_TRANSACTION_HAS_A_MAX_SIZE:
      CommonConstants.ERRORS.A_TRANSACTION_HAS_A_MAX_SIZE,
    BLOCK_ALREADY_PROCESSED: {
      httpCode: 400,
      uerr: { ucode: 2028, message: "Already processed" },
    },
    TOO_OLD_MEMBERSHIP: CommonConstants.ERRORS.TOO_OLD_MEMBERSHIP,
    TX_ALREADY_PROCESSED: {
      httpCode: 400,
      uerr: { ucode: 2030, message: "Transaction already processed" },
    },
    A_MORE_RECENT_MEMBERSHIP_EXISTS: {
      httpCode: 400,
      uerr: { ucode: 2031, message: "A more recent membership already exists" },
    },
  ...
  NETWORK: {
    ...
    STATUS_INTERVAL: {
      UPDATE: 6 * 12, // Every X blocks
      MAX: 20, // MAX Y blocks
    },
    ONION_ENDPOINT_REGEX: new RegExp(
      "(?:https?://)?(?:www)?(S*?.onion)(/[-w]*)*"
    ),
  },
  PROOF_OF_WORK: {
