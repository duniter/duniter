...
module.exports.add.post = function (req, res) {
  if(req.body && req.body.keytext && req.body.keysign){
    var extractPK = req.body.keytext.trim().match(/(-----BEGIN PGP[\s\S]*-----END PGP.*-----)/);
    var extractSIG = req.body.keysign.trim().match(/(-----BEGIN PGP[\s\S]*-----END PGP.*-----)/);
    if(extractPK && extractPK.length > 1 && extractSIG && extractSIG.length > 1){
      var asciiArmored = extractPK[1];
      var signature = extractSIG[1];
      var PublicKey = req.models.PublicKey;
      var pubKeys = [];
      PublicKey.verify(asciiArmored, signature, function (err) {
        if(!err){
          pubKeys.push(new PublicKey({ raw: asciiArmored }));
