...
export class RouterStream extends stream.Transform {
  ...
  async _write(obj: any, enc: any, done: any) {
    try {
      if (obj.joiners) {
        await this.route("block", obj, () =>
          this.getRandomInUPPeers(obj.issuer === this.peeringService.pubkey)()
        );
      } else if (obj.revocation) {
        await this.route("revocation", obj, () =>
          this.getRandomInUPPeers(obj.pubkey === this.peeringService.pubkey)()
        );
      } else if (obj.pubkey && obj.uid) {
        await this.route("identity", obj, () =>
          this.getRandomInUPPeers(obj.pubkey === this.peeringService.pubkey)()
        );
      } else if (obj.idty_uid) {
        await this.route("cert", obj, () =>
          this.getRandomInUPPeers(obj.pubkey === this.peeringService.pubkey)()
        );
      } else if (obj.userid) {
        await this.route("membership", obj, () =>
          this.getRandomInUPPeers(obj.issuer === this.peeringService.pubkey)()
        );
      } else if (obj.inputs) {
        await this.route("transaction", obj, () =>
          this.getRandomInUPPeers(
            obj.issuers.indexOf(this.peeringService.pubkey) !== -1
          )()
        );
      } else if (obj.endpoints) {
        await this.route("peer", obj, () =>
          this.getRandomInUPPeers(obj.pubkey === this.peeringService.pubkey)()
        );
      } else if (obj.from && obj.from == this.peeringService.pubkey) {
        // Route ONLY status emitted by this node
        await this.route("status", obj, () =>
          this.getTargeted(obj.to || obj.idty_issuer)()
        );
      ...
      if (
        ...
        e.uerr &&
        e.uerr.ucode ==
          constants.ERRORS.NEWER_PEER_DOCUMENT_AVAILABLE.uerr.ucode
      ) {
        this.logger.info(
          "Newer peer document available on the network for local node"
        );
      } else {
        this.logger.error(
          "Routing error: %s",
          e && (e.stack || e.message || (e.uerr && e.uerr.message) || e)
        );
      }
    }
    done && done();
  ...
  private getValidUpPeers(without: any, isSelfDocument: boolean) {
    return async () => {
      ...
      members = RouterStream.chooseXin(
        members,
        isSelfDocument
          ? constants.NETWORK.MAX_MEMBERS_TO_FORWARD_TO_FOR_SELF_DOCUMENTS
          : constants.NETWORK.MAX_MEMBERS_TO_FORWARD_TO
      );
      nonmembers = RouterStream.chooseXin(
        nonmembers,
        isSelfDocument
          ? constants.NETWORK.MAX_NON_MEMBERS_TO_FORWARD_TO_FOR_SELF_DOCUMENTS
          : constants.NETWORK.MAX_NON_MEMBERS_TO_FORWARD_TO
      );
      let mainRoutes: any = members
        .map((p: any) => (p.member = true) && p)
        .concat(nonmembers);
      let mirrors = await this.peeringService.mirrorBMAEndpoints();
      const peersToRoute: DBPeer[] = mainRoutes.concat(
        mirrors.map((mep, index) => {
          return {
