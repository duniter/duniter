...
export class EventWatcher extends events.EventEmitter implements Watcher {
  ...
  downloadPercent(pct?: number): number {
    return this.change(
      "downloadChange",
      (pct) => this.innerWatcher.downloadPercent(pct),
      pct
    );
  }
  ...
  storagePercent(pct?: number): number {
    return this.change(
      "storageChange",
      (pct) => this.innerWatcher.storagePercent(pct),
      pct
    );
  }
  ...
  appliedPercent(pct?: number): number {
    return this.change(
      "appliedChange",
      (pct) => this.innerWatcher.appliedPercent(pct),
      pct
    );
  }
  ...
  sbxPercent(pct?: number): number {
    return this.change(
      "sbxChange",
      (pct) => this.innerWatcher.sbxPercent(pct),
      pct
    );
  }
  ...
  peersPercent(pct?: number): number {
    return this.change(
      "peersChange",
      (pct) => this.innerWatcher.peersPercent(pct),
      pct
    );
  }
...
export class MultimeterWatcher implements Watcher {
  ...
  writeStatus(str: string) {
    ...
    this.charm.position(this.xPos, this.yPos).erase("end").write(str);
  }
...
export class LoggerWatcher implements Watcher {
  ...
  showProgress() {
    return this.logger.info(
      "Milestones %s%, Downloaded %s%, Applied %s%",
      this.savedPct,
      this.downPct,
      this.appliedPct
    );
