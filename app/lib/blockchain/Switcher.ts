...
export class Switcher<T extends SwitchBlock> {
  ...
  async tryToFork() {
    ...
    if (current) {
      ...
      let longestChain: null | T[] = await this.findLongestChain(
        current,
        suites
      );
      ...
      if (longestChain) {
        ...
        if (
          chainHEAD.number >= numberStart &&
          chainHEAD.medianTime >= timeStart
        ) {
   ...
   * @param current
   */
  async findPotentialSuitesHeads(current: {
    number: number;
    medianTime: number;
  }) {
  ...
  private async findPotentialSuites(numberStart: number, timeStart: number) {
    const suites: T[][] = [];
    const potentials: T[] = Underscore.sortBy(
      await this.dao.getPotentials(
        numberStart,
        timeStart,
      ...
      if (
        ...
        while (
          ...
          if (!knownForkBlocks[previousBlockstamp]) {
            previous = await this.dao.getBlockchainBlock(
              previousNumber,
              previousHash
            );
          }
          if (previous) {
            ...
            previous = await this.dao.getAbsoluteBlockInForkWindow(
              previousNumber,
              previousHash
            );
            if (previous) {
  ...
  private async findLongestChain(current: T, suites: T[][]) {
    ...
    for (const s of suites) {
      ...
      if (successfulBlocks.length) {
        ...
        const addedToHeadLevel =
          successfulBlocks[successfulBlocks.length - 1].number - current.number;
        this.logger &&
          this.logger.info(
      ...
      const successHEAD =
        successfulBlocks && successfulBlocks[successfulBlocks.length - 1];
      if (
