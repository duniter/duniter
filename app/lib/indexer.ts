...
export class Indexer {
  ...
  static localIndex(
    ...
    for (const inlineMS of block.joiners) {
      ...
      const matchesANewcomer =
        Underscore.filter(
          index,
          (row: IindexEntry) =>
            row.index == constants.I_INDEX && row.pub == ms.issuer
        ).length > 0;
      if (matchesANewcomer) {
        // Newcomer
        pushMindex(index, {
  ...
  @MonitorExecutionTime()
  static async quickCompleteGlobalScope(
    block: BlockDTO,
    conf: CurrencyConfDTO,
    bindex: DBHead[],
    iindex: IindexEntry[],
    mindex: MindexEntry[],
    cindex: CindexEntry[],
  ...
  static async completeGlobalScope(
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[],
    dal: FileDAL
    ...
    // BR_G03
    if (HEAD.number > 0) {
      HEAD.issuerIsMember = !!reduce(await dal.iindexDAL.reducable(HEAD.issuer))
        .member;
    } else {
      HEAD.issuerIsMember = !!reduce(
        Underscore.where(iindex, { pub: HEAD.issuer })
      ).member;
    }
    ...
    // BR_G08
    if (HEAD.number > 0) {
      HEAD.medianTime = Math.max(
        HEAD_1.medianTime,
        average(
          await range(1, Math.min(conf.medianTimeBlocks, HEAD.number), "time")
        )
      );
    } else {
    ...
    // BR_G10
    if (HEAD.number == 0) {
      HEAD.membersCount = count(
        Underscore.filter(iindex, (entry: IindexEntry) => entry.member === true)
      );
    } else {
      HEAD.membersCount =
        HEAD_1.membersCount +
        count(
          Underscore.filter(
            iindex,
            (entry: IindexEntry) => entry.member === true
          )
        ) -
        count(
          Underscore.filter(
            iindex,
            (entry: IindexEntry) => entry.member === false
          )
        );
    }
    ...
    // BR_G20
    await Promise.all(
      iindex.map(async (ENTRY: IindexEntry) => {
        if (ENTRY.op == constants.IDX_CREATE) {
          ENTRY.uidUnique =
            count(await dal.iindexDAL.findByUid(ENTRY.uid as string)) == 0;
        } else {
    ...
    await Promise.all(
      iindex.map(async (ENTRY: IindexEntry) => {
        if (ENTRY.op == constants.IDX_CREATE) {
          ENTRY.pubUnique =
            count(await dal.iindexDAL.findByPub(ENTRY.pub)) == 0;
        } else {
    ...
    await Promise.all(
      iindex.map(async (ENTRY: IindexEntry) => {
        if (ENTRY.member !== false) {
          ...
          ENTRY.excludedIsMember = !!reduce(
            await dal.iindexDAL.reducable(ENTRY.pub)
          ).member;
        }
      })
    );
        ...
        mindex.map(async (ENTRY: MindexEntry) => {
          if (ENTRY.revocation === null) {
            ...
            if (HEAD.medianTime >= 1498860000) {
              const rows = await dal.mindexDAL.findByPubAndChainableOnGt(
                ENTRY.pub,
                HEAD_1.medianTime
              );
      ...
      iindex.map(async (ENTRY: IindexEntry) => {
        ...
        const isBeingRevoked =
          count(
            Underscore.filter(
              mindex,
              (m) => !!(m.isBeingRevoked && m.pub == ENTRY.pub)
            )
          ) == 1;
    ...
    const oneIsOutdistanced = await checkPeopleAreNotOudistanced(
      ...
      // Newcomers
      Underscore.where(iindex, { op: constants.IDX_CREATE }).map(
    ...
    await Promise.all(
      mindex.map(async (ENTRY: MindexEntry) => {
        ENTRY.onRevoked =
          reduce(await dal.mindexDAL.reducable(ENTRY.pub)).revoked_on != null;
      })
    );
    ...
    await Promise.all(
      Underscore.filter(
        mindex,
        (entry: MindexEntry) =>
          entry.op == constants.IDX_UPDATE && entry.expired_on === 0
      ).map(async (ENTRY: MindexEntry) => {
        ENTRY.joinsTwice =
          reduce(await dal.iindexDAL.reducable(ENTRY.pub)).member == true;
      })
    );
    ...
    await Promise.all(
      mindex.map(async (ENTRY: MindexEntry) => {
        if (!ENTRY.revoked_on) {
          ...
          ENTRY.revocationSigOK = await sigCheckRevoke(
            ENTRY,
            dal,
            block.currency
          );
    ...
    if (HEAD.number > 0) {
      await Promise.all(
        cindex.map(async (ENTRY: CindexEntry) => {
          const rows = await dal.cindexDAL.findByIssuerAndChainableOnGt(
            ENTRY.issuer,
            HEAD_1.medianTime
          );
          ENTRY.unchainables = count(rows);
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        ENTRY.stock = count(
          await dal.cindexDAL.getValidLinksFrom(ENTRY.issuer)
        );
      })
    );
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        ENTRY.fromMember = !!reduce(await dal.iindexDAL.reducable(ENTRY.issuer))
          .member;
      })
    );
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        ENTRY.toMember = !!reduce(await dal.iindexDAL.reducable(ENTRY.receiver))
          .member;
      })
    );
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        ENTRY.toNewcomer =
          count(
            Underscore.where(iindex, { member: true, pub: ENTRY.receiver })
          ) > 0;
      })
    );
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        ENTRY.toLeaver = !!reduce(await dal.mindexDAL.reducable(ENTRY.receiver))
          .leaving;
      })
    );
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        const reducable = await dal.cindexDAL.findByIssuerAndReceiver(
          ENTRY.issuer,
          ENTRY.receiver
        );
        ENTRY.isReplay =
          count(reducable) > 0 && reduce(reducable).expired_on === 0;
        if (HEAD.number > 0 && HEAD_1.version > 10) {
          ENTRY.isReplayable =
            count(reducable) === 0 ||
            reduce(reducable).replayable_on < HEAD_1.medianTime;
        } else {
    ...
    await Promise.all(
      cindex.map(async (ENTRY: CindexEntry) => {
        ENTRY.sigOK = await checkCertificationIsValid(
          block,
          ENTRY,
          async (block: BlockDTO, pub: string, dal: FileDAL) => {
            let localInlineIdty = block.getInlineIdentity(pub);
            if (localInlineIdty) {
              return IdentityDTO.fromInline(localInlineIdty);
            }
            const idty = await dal.getWrittenIdtyByPubkeyForCertificationCheck(
              pub
            );
            if (!idty) {
              return null;
    ...
    // BR_G102
    await Promise.all(
      Underscore.where(sindex, { op: constants.IDX_UPDATE }).map(
        async (ENTRY: SindexEntry) => {
          if (
            HEAD.number == 0 &&
            ENTRY.created_on ==
            ...
            let ref = await dal.getAbsoluteValidBlockInForkWindowByBlockstamp(
              ENTRY.created_on as string
            );
    ...
    const getInputLocalFirstOrFallbackGlobally = async (
      sindex: SindexEntry[],
      ENTRY: SindexEntry
    ): Promise<SimpleTxInput> => {
      let source: SimpleTxInput | null = Underscore.filter(
        sindex,
        (src) =>
          src.identifier == ENTRY.identifier &&
          src.pos == ENTRY.pos &&
          src.conditions !== "" &&
          src.op === constants.IDX_CREATE
      )[0];
      if (!source) {
    ...
    await Promise.all(
      Underscore.where(sindex, { op: constants.IDX_UPDATE }).map(
        async (ENTRY: SindexEntry) => {
          const source = await getInputLocalFirstOrFallbackGlobally(
            sindex,
            ENTRY
          );
    ...
    await Promise.all(
      Underscore.where(sindex, { op: constants.IDX_UPDATE }).map(
        async (ENTRY: SindexEntry) => {
          ...
          ENTRY.isTimeLocked =
            ENTRY.written_time - source.written_time < ENTRY.locktime;
        }
      )
    );
  ...
  // BR_G04
  static async prepareIssuersCount(
    HEAD: DBHead,
    range: Ranger,
    HEAD_1: DBHead
  ) {
    if (HEAD.number == 0) {
      ...
      HEAD.issuersCount = count(
        uniq(Underscore.pluck(await range(1, HEAD_1.issuersFrame), "issuer"))
      );
    }
  }
  ...
  static async prepareAvgBlockSize(HEAD: DBHead, range: Ranger) {
    HEAD.avgBlockSize = average(
      Underscore.pluck(await range(1, HEAD.issuersCount), "bsize")
    );
  }

  // BR_G09
  static prepareDiffNumber(
    HEAD: DBHead,
    HEAD_1: DBHead,
    conf: CurrencyConfDTO
  ) {
  ...
  static prepareDividend(HEAD: DBHead, HEAD_1: DBHead, conf: CurrencyConfDTO) {
    ...
    if (HEAD.number == 0) {
      ...
      HEAD.dividend = Math.ceil(
        HEAD_1.dividend +
          (Math.pow(conf.c, 2) *
            Math.ceil(HEAD_1.massReeval / Math.pow(10, previousUB))) /
            HEAD.membersCount /
  ...
  static prepareMass(HEAD: DBHead, HEAD_1: DBHead) {
    ...
    if (HEAD.number == 0) {
      ...
      HEAD.mass =
        HEAD_1.mass +
        HEAD.dividend * Math.pow(10, HEAD.unitBase) * HEAD.membersCount;
    } else {
  ...
  // BR_G16
  static async prepareSpeed(
    HEAD: DBHead,
    head: (n: number) => Promise<DBHead>,
    conf: CurrencyConfDTO
  ) {
    if (HEAD.number == 0) {
      HEAD.speed = 0;
  ...
  // BR_G18
  static async preparePersonalizedPoW(
    HEAD: DBHead,
    HEAD_1: DBHead,
    ...
    if (HEAD.number == 0) {
      ...
      const blocksInFrame = Underscore.filter(
        ranged,
        (b: DBHead) => b.number <= HEAD_1.number
      );
      const issuersInFrame = blocksInFrame.map((b) => b.issuer);
      blocksOfIssuer = Underscore.filter(
        blocksInFrame,
    ...
    // V0.6 Hardness
    const PERSONAL_EXCESS = Math.max(
      0,
      (nbPersonalBlocksInFrame + 1) / medianOfBlocksInFrame - 1
    );
    const PERSONAL_HANDICAP = Math.floor(
      Math.log(1 + PERSONAL_EXCESS) / Math.log(1.189)
    );
    HEAD.issuerDiff =
      Math.max(
        HEAD.powMin,
        HEAD.powMin *
          Math.floor(
            (conf.percentRot * nbPreviousIssuers) / (1 + nbBlocksSince)
          )
      ) + PERSONAL_HANDICAP;
  ...
  // BR_G19
  static async prepareIdentitiesAge(
    iindex: IindexEntry[],
    HEAD: DBHead,
    HEAD_1: DBHead,
    conf: CurrencyConfDTO,
  ...
  // BR_G22
  static async prepareMembershipsAge(
    mindex: MindexEntry[],
    HEAD: DBHead,
    HEAD_1: DBHead,
    conf: CurrencyConfDTO,
  ...
  // BR_G37
  static async prepareCertificationsAge(
    cindex: CindexEntry[],
    HEAD: DBHead,
    HEAD_1: DBHead,
    conf: CurrencyConfDTO,
    ...
    await Promise.all(
      cindex.map(async (ENTRY) => {
        if (HEAD.number == 0) {
          ENTRY.age = 0;
        } else {
          let ref =
            ENTRY.created_on_ref || (await dal.getTristampOf(ENTRY.created_on));
          if (ref) {
  ...
  static ruleVersion(HEAD: DBHead, HEAD_1: DBHead) {
    if (HEAD.number > 0) {
      return (
        HEAD.version == HEAD_1.version || HEAD.version == HEAD_1.version + 1
      );
    }
    return true;
  }
  ...
  static rulePreviousHash(block: BlockDTO, HEAD: DBHead) {
    return (
      block.previousHash == HEAD.previousHash ||
      (!block.previousHash && !HEAD.previousHash)
    );
  }
  ...
  static rulePreviousIssuer(block: BlockDTO, HEAD: DBHead) {
    return (
      block.previousIssuer == HEAD.previousIssuer ||
      (!block.previousIssuer && !HEAD.previousIssuer)
    );
  }
  ...
  static ruleProofOfWork(HEAD: DBHead) {
    ...
    const powRegexp = new RegExp(
      "^0{" + nbZerosReq + "}" + "[0-" + highMark + "]"
    );
    try {
      if (!HEAD.hash.match(powRegexp)) {
        const match = HEAD.hash.match(/^0*/);
        const givenZeros = Math.max(
          0,
          Math.min(nbZerosReq, (match && match[0].length) || 0)
        );
        ...
        throw Error(
          "Wrong proof-of-work level: given " +
            ...
            "', required was " +
            nbZerosReq +
            " zeros and an hexa char between [0-" +
            highMark +
            "]"
        );
      }
  ...
  static async ruleToBeKickedArePresent(iindex: IindexEntry[], dal: FileDAL) {
    ...
    for (const toKick of toBeKicked) {
      if (
        count(
          Underscore.where(iindex, { pub: toKick, isBeingKicked: true })
        ) !== 1
      ) {
        return false;
      }
    }
    const beingKicked = Underscore.filter(
      iindex,
      (i: IindexEntry) => i.member === false
    );
  ...
  // BR_G91
  static async ruleIndexGenDividend(
    HEAD: DBHead,
  ...
  static async ruleIndexGarbageSmallAccounts(
    HEAD: DBHead,
    transactions: SindexEntry[],
    dividends: SimpleUdEntryForWallet[],
    dal: AccountsGarbagingDAL
  ) {
    ...
    const accounts = Object.keys(
      sindex.reduce((acc: { [k: string]: boolean }, src) => {
        acc[src.conditions] = acc[src.conditions] || src.srcType === "T"; // We don't touch accounts that only received an UD
        return acc;
      }, {})
    );
    const wallets: { [k: string]: Promise<DBWallet> } = accounts.reduce(
      (map: { [k: string]: Promise<DBWallet> }, acc) => {
    ...
    for (const account of accounts) {
      const localAccountEntries = Underscore.filter(
        sindex,
        (src) => src.conditions == account
      );
      const wallet = await wallets[account];
      const balance = wallet.balance;
      const variations = localAccountEntries.reduce(
        (sum: number, src: SindexEntry) => {
          if (src.op === "CREATE") {
      ...
      if (balance + variations < 0) {
        ...
        balance + variations <
        constants.ACCOUNT_MINIMUM_CURRENT_BASED_AMOUNT *
  ...
  static async ruleIndexGenCertificationExpiry(HEAD: DBHead, dal: FileDAL) {
    const expiries = [];
    const certs = await dal.cindexDAL.findExpiresOnLteNotExpiredYet(
      HEAD.medianTime
    );
  ...
  static async ruleIndexGenMembershipExpiry(HEAD: DBHead, dal: FileDAL) {
    return (
      await dal.mindexDAL.findPubkeysThatShouldExpire(HEAD.medianTime)
    ).map((MS) => {
      return {
        op: "UPDATE",
        pub: MS.pub,
  ...
  // BR_G94
  static async ruleIndexGenExclusionByMembership(
    HEAD: DBHead,
    mindex: MindexEntry[],
    dal: FileDAL
  ) {
    const exclusions = [];
    const memberships = Underscore.filter(
      mindex,
      (entry) => !!entry.expired_on
    );
  ...
  // BR_G95
  static async ruleIndexGenExclusionByCertificatons(
    HEAD: DBHead,
    cindex: CindexEntry[],
    iindex: IindexEntry[],
    conf: ConfDTO,
    ...
    const expiredCerts = Underscore.filter(
      cindex,
      (c: CindexEntry) => c.expired_on > 0
    );
    for (const CERT of expiredCerts) {
      const just_expired = Underscore.filter(
        cindex,
        (c: CindexEntry) => c.receiver == CERT.receiver && c.expired_on > 0
      );
      const just_received = Underscore.filter(
        cindex,
        (c: CindexEntry) => c.receiver == CERT.receiver && c.expired_on == 0
      );
      const non_expired_global = await dal.cindexDAL.getValidLinksTo(
        CERT.receiver
      );
      if (
        count(non_expired_global) - count(just_expired) + count(just_received) <
        conf.sigQty
      ) {
        const isInExcluded = Underscore.filter(
          iindex,
          (i: IindexEntry) => i.member === false && i.pub === CERT.receiver
        )[0];
        const isInKicked = Underscore.filter(
          exclusions,
          (e) => e.pub === CERT.receiver
        )[0];
  ...
  static async ruleIndexGenImplicitRevocation(HEAD: DBHead, dal: FileDAL) {
    const revocations = [];
    const pending = await dal.mindexDAL.findRevokesOnLteAndRevokedOnIsNull(
      HEAD.medianTime
    );
  ...
  // BR_G104
  static async ruleIndexCorrectMembershipExpiryDate(
    HEAD: DBHead,
    mindex: MindexEntry[],
    dal: FileDAL
  ) {
  ...
  // BR_G105
  static async ruleIndexCorrectCertificationExpiryDate(
    HEAD: DBHead,
    cindex: CindexEntry[],
    dal: FileDAL
  ) {
    for (const CERT of cindex.filter((c) => c.op === "CREATE")) {
      ...
      if (HEAD.number == 0) {
        ...
        basedBlock =
          CERT.created_on_ref ||
          ((await dal.getTristampOf(CERT.created_on)) as DBBlock); // We are sure at this point in the rules
      }
  ...
  static DUP_HELPERS = {
    ...
    reduceOrNull,
    reduceBy: reduceBy,
    getMaxBlockSize: (HEAD: DBHead) =>
      Math.max(500, Math.ceil(1.1 * HEAD.avgBlockSize)),
...
export function reduceForDBTrimming<T extends { writtenOn: number }>(
  records: T[],
  belowNumber: number
): T[] {
  if (records.length === 0) {
...
export function reduceBy<T extends IndexEntry>(
  reducables: T[],
  properties: (keyof T)[]
): T[] {
  ...
  return Underscore.values(reduced).map((value) =>
    Indexer.DUP_HELPERS.reduce(value)
  );
}

async function checkPeopleAreNotOudistanced(
  pubkeys: string[],
  newLinks: { [k: string]: string[] },
  newcomers: string[],
  conf: ConfDTO,
  dal: FileDAL
) {
  ...
  for (const pubkey of pubkeys) {
    ...
    let isOutdistanced = wotb.isOutdistanced(
      nodeID,
      dSen,
      conf.stepMax,
      conf.xpercent
...
async function getNodeIDfromPubkey(
  nodesCache: { [k: string]: number },
  pubkey: string,
  dal: FileDAL
) {
...
async function sigCheckRevoke(
  entry: MindexEntry,
  dal: FileDAL,
  currency: string
) {
  try {
    let pubkey = entry.pub,
      sig = entry.revocation || "";
...
async function checkCertificationIsValid(
  block: BlockDTO,
  cert: CindexEntry,
  findIdtyFunc: (
    b: BlockDTO,
  ...
  if (block.number == 0 && cert.created_on != 0) {
    ...
    try {
      ...
      let current =
        block.number == 0 ? null : await dal.getCurrentBlockOrNull();
      if (!idty) {
        throw Error("Identity does not exist for certified");
      } else if (
        current &&
        current.medianTime > basedBlock.medianTime + conf.sigValidity
      ) {
