...
export class BIndexDAL extends AbstractSQLite<DBHead> {
  ...
  async head(n: number): Promise<DBHead> {
    ...
    const headRecords = await this.query(
      "SELECT * FROM " + this.table + " ORDER BY number DESC LIMIT 1 OFFSET ?",
      [n - 1]
    );
   ...
   */
  async tail() {
    const tailRecords = await this.query(
      "SELECT * FROM " + this.table + " ORDER BY number ASC LIMIT 1",
      []
    );
  ...
  range(n: number, m: number) {
    const count = m - n + 1;
    return this.query(
      "SELECT * FROM " + this.table + " ORDER BY number DESC LIMIT ? OFFSET ?",
      [count, n - 1]
    );
  }
  ...
  trimBlocks(maxnumber: number) {
    return this.exec(
      "DELETE FROM " + this.table + " WHERE number < " + maxnumber
    );
  }
}
