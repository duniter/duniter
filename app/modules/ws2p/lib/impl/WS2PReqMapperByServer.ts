...
export class WS2PReqMapperByServer implements WS2PReqMapper {
  ...
  async getBlock(number: number): Promise<BlockDTO> {
    return Promise.resolve(
      BlockDTO.fromJSONObject(await this.server.dal.getFullBlockOf(number))
    );
  }
  ...
  async getRequirementsOfPending(minsig: number): Promise<any> {
    ...
    const all = await this.server.BlockchainService.requirementsOfIdentities(
      identities,
      false
    );
    return {
  ...
  async getKnownPeers(): Promise<PeerDTO[]> {
    return (await this.server.dal.findAllPeersBut([])).map((p) =>
      PeerDTO.fromDBPeer(p)
    );
  }
}
