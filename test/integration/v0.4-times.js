...
describe("Protocol 0.4 Times", function() {
  ...
  it('a V3 block should not accept a time = medianTime + avgGenTime * √(1,066)', () => co(function*() {
    yield s1.commit({ version: 3, medianTime: now, time: Math.ceil(now + conf.avgGenTime * Math.sqrt(1.066)) });
    yield s1.revert();
  }));
  ...
  it('a V4 block should not accept a time = medianTime + avgGenTime * 1.189', () => co(function*() {
    yield s1.commit({ version: 4, medianTime: now, time: Math.ceil(now + conf.avgGenTime * 1.189) });
    yield s1.revert();
  }));
