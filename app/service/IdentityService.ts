...
export class IdentityService extends FIFOService {
  ...
  async findMember(search: string) {
    ...
    if (idty) {
      ...
      memberships = memberships.concat(
        mssFromMindex.map((ms: MindexEntry) => {
  ...
  submitIdentity(
    ...
    return this.pushFIFO<DBIdentity>(hash, async () => {
      ...
      try {
        ...
        let existing = await this.dal.getGlobalIdentityByHashForExistence(
          toSave.hash
        );
        if (existing) {
          ...
          let used = await GLOBAL_RULES_HELPERS.checkExistsPubkey(
            idty.pubkey,
            this.dal
          );
          if (used) {
          ...
          used = await GLOBAL_RULES_HELPERS.checkExistsUserID(
            idty.uid,
            this.dal
          );
          if (used) {
          ...
          await GLOBAL_RULES_FUNCTIONS.checkIdentitiesAreWritable(
            {
              identities: [idtyObj.inline()],
              version:
                (current && current.version) || constants.BLOCK_GENESIS_VERSION,
            },
            this.conf,
            this.dal
          );
          if (byAbsorption !== BY_ABSORPTION) {
            if (
              !(await this.dal.idtyDAL.sandbox.acceptNewSandBoxEntry(
                {
                  certsCount: 0,
                  issuers: [idty.pubkey],
  ...
  async submitCertification(obj: any): Promise<CertificationDTO> {
    ...
    const potentialNext = BlockDTO.fromJSONObject({
    ...
    return this.pushFIFO<CertificationDTO>(hash, async () => {
      ...
      try {
        await GLOBAL_RULES_HELPERS.checkCertificationIsValidInSandbox(
          cert,
          potentialNext,
          () => Promise.resolve(idty),
          this.conf,
          this.dal
        );
      } catch (e) {
      ...
      if (!anErr) {
        try {
          let basedBlock: Tristamp | null = await this.dal.getTristampOf(
            cert.block_number
          );
          ...
          if (!existingCert) {
            if (
              !(await this.dal.certDAL
                .getSandboxForKey(cert.from)
  ...
  submitRevocation(obj: any) {
    ...
    return this.pushFIFO<RevocationDTO>(hash, async () => {
      try {
        ...
        const existing = await this.dal.getGlobalIdentityByHashForRevocation(
          obj.hash
        );
        if (existing) {
          // Modify
