...
export class MembershipService extends FIFOService {
  ...
  submitMembership(ms: any) {
    ...
    return this.pushFIFO<MembershipDTO>(hash, async () => {
      ...
      const basedBlock = await GLOBAL_RULES_HELPERS.checkMembershipBlock(
        entry,
        current,
        this.conf,
        this.dal
      ...
      if (
        !(await this.dal.msDAL.sandbox.acceptNewSandBoxEntry(
          {
      ...
      const expires_on = basedBlock
        ? basedBlock.medianTime + this.conf.msWindow
        : 0;
