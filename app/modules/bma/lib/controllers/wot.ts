...
export class WOTBinding extends AbstractController {
  async lookup(req: any): Promise<HttpLookup> {
    ...
    identities.forEach(
      (idty, index) => (identities[index] = DBIdentity.copyFromExisting(idty))
    );
    // Prepare some data to avoid displaying expired certifications
    for (const idty of identities) {
      const certs: any[] = await this.server.dal.certsToTarget(
        idty.pubkey,
        idty.getTargetHash()
      );
      const validCerts = [];
      for (const cert of certs) {
        ...
        if (member) {
          ...
          const potentials = await this.IdentityService.getPendingFromPubkey(
            cert.from
          );
      ...
      for (let j = 0; j < signed.length; j++) {
        ...
        cert.idty = await this.server.dal.getGlobalIdentityByHashForLookup(
          cert.target
        );
        if (cert.idty) {
          ...
          this.logger.debug(
            "A certification to an unknown identity was found (%s => %s)",
            cert.from,
            cert.to
          );
        }
    ...
    for (const cert of certs) {
      ...
      if (certifier) {
        ...
        theCerts.push({
          ...
          written:
            cert.written_block !== null && cert.written_hash
              ? {
  ...
  async requirementsOfPending(req: any): Promise<HttpRequirements> {
    ...
    let identities: IdentityForRequirements[] = (
      await this.server.dal.idtyDAL.query(
    ...
    const all = await this.BlockchainService.requirementsOfIdentities(
      identities,
      false
    );
    if (!all || !all.length) {
  ...
  async add(req: any): Promise<HttpIdentity> {
    const res = await this.pushEntity(req, http2raw.identity, (raw: string) =>
      this.server.writeRawIdentity(raw)
    );
    return {
      pubkey: res.pubkey,
      uids: [],
  ...
  async certify(req: any): Promise<HttpCert> {
    const res = await this.pushEntity(
      req,
      http2raw.certification,
      (raw: string) => this.server.writeRawCertification(raw)
    );
  ...
  async revoke(req: any): Promise<HttpResult> {
    const res = await this.pushEntity(req, http2raw.revocation, (raw: string) =>
      this.server.writeRawRevocation(raw)
    );
    return {
      result: true,
    };
    ...
    const json = {
      memberships: memberships.map((ms: DBMembership) => {
        return {
          ...
          written:
            !ms.written_number && ms.written_number !== 0
              ? null
              : ms.written_number,
        };
      }),
