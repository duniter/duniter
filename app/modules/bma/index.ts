...
export const BmaDependency = {
  duniter: {
    cliOptions: [
      ...
      {
        value: "--ipv4 <address>",
        desc: "IPv4 interface to listen for requests",
      },
      {
        value: "--ipv6 <address>",
        desc: "IPv6 interface to listen for requests",
      },
      {
        value: "--remoteh <host>",
        desc: "Remote interface others may use to contact this node",
      },
      { value: "--remote4 <host>", desc: "Remote interface for IPv4 access" },
      { value: "--remote6 <host>", desc: "Remote interface for IPv6 access" },
      {
        value: "--remotep <port>",
        desc: "Remote port others may use to contact this node",
      },
    ],
    ...
    wizard: {
      ...
      "network-reconfigure": async (
        ...
        if (!networkWizardDone) {
          ...
          await Q.nbind(networkReconfiguration, null)(
            conf,
            program.autoconf,
            logger,
            program.noupnp
    ...
    config: {
      onLoading: async (conf: NetworkConfDTO, program: any, logger: any) => {
        ...
        if (conf.bmaWithCrawler === undefined) {
          conf.bmaWithCrawler = false;
        }
        ...
        if (program.remotep !== undefined)
          conf.remoteport = parseInt(program.remotep);
        ...
        if (program.bmaWithoutCrawler !== undefined)
          conf.bmaWithCrawler = false;
        ...
        if (
          conf.ipv4 &&
          !(conf.remoteipv4 || conf.remotehost || conf.remoteipv6)
        ) {
      ...
      input: (server: Server, conf: NetworkConfDTO, logger: any) => {
        ...
        if (!conf.nobma) {
          if (!conf.ipv4 && !conf.ipv6) {
            throw new Error(
              "BMA: no interface to listen to. Provide ipv4/ipv6 interface or deactivate BMA"
            );
          }
...
export class BMAPI extends stream.Transform {
  ...
  startService = async () => {
    ...
    if (this.server.conf.upnp) {
      try {
        this.upnpAPI = await Upnp(
          this.server.conf.port,
          this.server.conf.remoteport,
          this.logger,
          this.server.conf
        );
...
export function networkReconfiguration(
  conf: NetworkConfDTO,
  autoconf: boolean,
  logger: any,
  noupnp: boolean,
  ...
  async.waterfall(
    [
      ...
      function (upnpSuccess: boolean, upnpConf: NetworkConfDTO, next: any) {
        ...
        const remoteOpertions = getRemoteNetworkOperations(
          conf,
          upnpConf.remoteipv4
        );
        ...
        if (upnpSuccess) {
          ...
          if (autoconf) {
            ...
            return async.waterfall(
              useUPnPOperations.concat(dnsOperations),
              next
            );
          }
          choose(
            "UPnP is available: duniter will be bound: \n  from " +
              local +
              "\n  to " +
              remote +
            ...
            function () {
              ...
              conf.ipv6 = conf.remoteipv6 = Network.getBestLocalIPv6();
              async.waterfall(useUPnPOperations.concat(dnsOperations), next);
            },
            function () {
              // No: want to change
              async.waterfall(
          ...
          if (autoconf) {
            ...
            return async.waterfall(
              localOperations
                ...
                .concat([
                  function (confDone: any) {
                    conf.remoteipv4 = conf.ipv4;
                    conf.remoteipv6 = conf.ipv6;
                    conf.remoteport = conf.port;
                    logger.info(
                      "Local & Remote IPv4: %s",
                      [conf.ipv4, conf.port].join(":")
                    );
                    logger.info(
                      "Local & Remote IPv6: %s",
                      [conf.ipv6, conf.port].join(":")
                    );
                    confDone();
                  },
                ]),
          ...
          choose(
            "UPnP is *not* available: is this a public server (like a VPS)?",
            true,
            function () {
              // Yes: local configuration = remote configuration
              async.waterfall(
            ...
            function () {
              ...
              async.waterfall(
                localOperations.concat(remoteOpertions).concat(dnsOperations),
                next
              );
            }
          );
  ...
  async.waterfall(
    [
      ...
      function (upnpSuccess: boolean, upnpConf: NetworkConfDTO, next: any) {
        ...
        async.waterfall(
          operations.concat(getHostnameOperations(conf, logger, false)),
          next
        );
      },
    ],
...
function getLocalNetworkOperations(
  conf: NetworkConfDTO,
    ...
    function (next: any) {
      ...
      osInterfaces.forEach(function (netInterface: any) {
        ...
        filtered.forEach(function (addr: any) {
          ...
          if (addr.scopeid) address += "%" + netInterface.name;
          let nameSuffix = "";
...
function getRemoteNetworkOperations(
  conf: NetworkConfDTO,
        ...
        .prompt([
          {
            ...
            validate: function (input: any) {
              return !!(
                input && input.toString().match(BMAConstants.IPV4_REGEXP)
              );
            },
          },
        ])
...
function getHostnameOperations(
  conf: NetworkConfDTO,
  logger: any,
  autoconf = false
) {
  return [
    function (next: any) {
      ...
      choose(
        "Does this server has a DNS name?",
        !!conf.remotehost,
        function () {
          // Yes
          simpleValue(
...
function getUseUPnPOperations(
  ...
  return [
    function (next: any) {
      ...
      choose(
        "UPnP is available: use automatic port mapping? (easier)",
        conf.upnp,
        function () {
          conf.upnp = true;
          next();
