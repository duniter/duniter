...
const getHomePath = (
  ...
  directory: string | null = null
) => path.normalize(getUserHome(directory) + "/") + getDomain(profile);

const getUserHome = (directory: string | null = null) =>
...
export const MemFS = (
  initialTree: { [folder: string]: { [file: string]: string } } = {}
): FileSystem => {
...
export const Directory = {
  ...
  GET_FILE_PATH: (fileSubPath: string, home = "") =>
    path.join(home || Directory.INSTANCE_HOME, fileSubPath),
  ...
  getHome: (profile: string | null = null, directory: string | null = null) =>
    getHomePath(profile, directory),
