...
export class UDBinding extends AbstractController {
  ...
  async getHistoryBetweenBlocks(req: any) {
    ...
    return this.getUDSources(pubkey, (results: any) => {
      results.history.history = Underscore.filter(
        results.history.history,
        function (ud: any) {
          return ud.block_number >= from && ud.block_number <= to;
        }
      );
      return results;
    });
  ...
  async getHistoryBetweenTimes(req: any) {
    ...
    return this.getUDSources(pubkey, (results: any) => {
      results.history.history = Underscore.filter(
        results.history.history,
        function (ud: any) {
          return ud.time >= from && ud.time <= to;
        }
      );
      return results;
    });
  ...
  private async getUDSources(pubkey: string, filter: any) {
    ...
    Underscore.keys(history).map((key: any) => {
      history[key].map((src: any, index: number) => {
        ...
        Underscore.extend(history[key][index], {
          block_number: src && src.block_number,
          time: src && src.time,
        });
      });
    });
