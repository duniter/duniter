...
export const duniterDocument2str = (type: DuniterDocument) => {
  switch (type) {
    case DuniterDocument.ENTITY_BLOCK:
      return "block";
    case DuniterDocument.ENTITY_IDENTITY:
      return "identity";
    case DuniterDocument.ENTITY_CERTIFICATION:
      return "certification";
    case DuniterDocument.ENTITY_REVOCATION:
      return "revocation";
    case DuniterDocument.ENTITY_MEMBERSHIP:
      return "membership";
    case DuniterDocument.ENTITY_TRANSACTION:
      return "transaction";
    case DuniterDocument.ENTITY_PEER:
      return "peer";
    default:
      return "";
  }
...
export const CommonConstants = {
  ...
  ERRORS: {
    ...
    DOCUMENT_BEING_TREATED: {
      httpCode: 400,
      uerr: { ucode: 1015, message: "Document already under treatment" },
    },

    // Business errors
    WRONG_UNLOCKER: {
      httpCode: 400,
      uerr: { ucode: 2013, message: "Wrong unlocker in transaction" },
    },
    LOCKTIME_PREVENT: {
      httpCode: 400,
      uerr: { ucode: 2014, message: "Locktime not elapsed yet" },
    },
    SOURCE_ALREADY_CONSUMED: {
      httpCode: 400,
      uerr: { ucode: 2015, message: "Source already consumed" },
    },
    WRONG_AMOUNTS: {
      httpCode: 400,
      uerr: { ucode: 2016, message: "Sum of inputs must equal sum of outputs" },
    },
    WRONG_OUTPUT_BASE: {
      httpCode: 400,
      uerr: { ucode: 2017, message: "Wrong unit base for outputs" },
    },
    CANNOT_ROOT_BLOCK_NO_MEMBERS: {
      httpCode: 400,
      uerr: {
        ucode: 2018,
        message: "Wrong new block: cannot make a root block without members",
      },
    },
    IDENTITY_WRONGLY_SIGNED: {
      ...
      uerr: {
        ucode: 2019,
        message: "Weird, the signature is wrong and in the database.",
      },
    },
    TOO_OLD_IDENTITY: {
      httpCode: 400,
    ...
    PEER_DOCUMENT_ALREADY_KNOWN: {
      httpCode: 400,
      uerr: { ucode: 2023, message: "Peer document already known" },
    },
    ...
    BLOCKSTAMP_DOES_NOT_MATCH_A_BLOCK: {
      httpCode: 400,
      uerr: { ucode: 2026, message: "Blockstamp does not match a block" },
    },
    A_TRANSACTION_HAS_A_MAX_SIZE: {
      ...
      uerr: {
        ucode: 2027,
        message:
          "A transaction has a maximum size of " +
          MAXIMUM_LEN_OF_COMPACT_TX +
          " lines",
      },
    },
    TOO_OLD_MEMBERSHIP: {
      httpCode: 400,
      uerr: { ucode: 2029, message: "Too old membership." },
    },
    MAXIMUM_LEN_OF_OUTPUT: {
      httpCode: 400,
      uerr: {
        ucode: 2032,
        message:
          "A transaction output has a maximum size of " +
          MAXIMUM_LEN_OF_OUTPUT +
          " characters",
      },
    },
    MAXIMUM_LEN_OF_UNLOCK: {
      ...
      uerr: {
        ucode: 2033,
        message:
          "A transaction unlock has a maximum size of " +
          MAXIMUM_LEN_OF_UNLOCK +
          " characters",
      },
    },
    WRONG_CURRENCY: {
      httpCode: 400,
      uerr: { ucode: 2500, message: "Wrong currency" },
    },
    WRONG_POW: {
      httpCode: 400,
      uerr: { ucode: 2501, message: "Wrong proof-of-work" },
    },
    OUT_OF_FORK_WINDOW: {
      httpCode: 400,
      uerr: { ucode: 2501, message: "Out of fork window" },
    },

    WRONG_SIGNATURE_FOR_CERT: {
      httpCode: 400,
      uerr: { ucode: 3000, message: "Wrong signature for certification" },
    },
  },
  ...
  BLOCK: {
    ...
    BLOCK_ISSUERS_FRAME_VAR: find(
      "IssuersFrameVar: (" + RELATIVE_INTEGER + ")"
    ),
  ...
  WAIT_P2P_CANDIDATE_HEARTBEAT: 30 * 1000, // Wait X seconds for a node to answer about its state
