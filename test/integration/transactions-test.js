...
describe("Testing transactions", function() {
  ...
  describe("Chaining", function(){
    ...
    it('with MULTISIG', () => co(function *() {
      ...
      let tx2 = yield tic.prepareUTX(tx1, ['SIG(0)'], [{ qty: 200, lock: '(XHX(8AFC8DF633FC158F9DB4864ABED696C1AA0FE5D617A7B5F7AB8DE7CA2EFCD4CB) && SIG(' + toc.pub + ')) || (SIG(' + tic.pub + ') && SIG(' + toc.pub + '))'  }], { comment: 'cross1' });
      yield unit.shouldNotFail(toc.sendTX(tx2));
      yield node2.commitP(); // TX2 commited
      let tx3 = yield tic.prepareUTX(tx2, ['XHX(1872767826647264) SIG(0)'], [{ qty: 200, lock: 'SIG(' + toc.pub + ')' }], { comment: 'wrong'});
