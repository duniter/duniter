  ...
  _verify(obj: any) {
    ...
    if (!err) {
      ...
      if (!obj.pubkey || !obj.pubkey.match(CommonConstants.BASE58))
        err = {
          code: codes.BAD_FINGERPRINT,
          message: "Incorrect PublicKey field",
        };
    }
    if (!err) {
      // Block
    ...
    if (!err) {
      ...
      for (const ep of obj.endpoints || []) {
        if (!err && ep.length > 255) {
          err = {
            code: codes.TOO_LONG_ENDPOINT,
            message: "An endpoint has maximum 255 characters length.",
          };
        }
      }
    }
    ...
    if (bma) {
      ...
      if (!err) {
        ...
        if (!bma.dns && !bma.ipv4 && !bma.ipv6)
          err = {
            code: codes.NO_IP_GIVEN,
            message: "It must be given at least DNS or one IP, either v4 or v6",
          };
      }
      if (!err) {
        // Port
        if (bma.port && !(bma.port + "").match(/^\d+$/))
          err = {
            code: codes.BAD_PORT,
            message: "Port must be provided and match an integer format",
          };
      }
    }
