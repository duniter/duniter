...
export class WS2PServer extends events.EventEmitter {
  ...
  private constructor(
    ...
    private shouldAcceptConnection: (
      pubkey: string,
      isSync: boolean,
      syncConnectedPubkeys: string[],
      connectedPubkeys: string[]
    ) => Promise<boolean>,
    public keyPriorityLevel: (
      pubkey: string,
  ...
  get maxLevel2Peers() {
    if (
      ...
      this.server.conf.ws2p.maxPublic !== undefined &&
      this.server.conf.ws2p.maxPublic !== null
    ) {
  ...
  private listenToWebSocketConnections(messageHandler: WS2PMessageHandler) {
    ...
    this.wss.on("connection", async (ws: any) => {
      this.server.logger.info(
        "WS2P %s: new incoming connection from %s:%s!",
        this.server.conf.pair.pub,
        ws._sender._socket._handle.owner.remoteAddress,
        ws._sender._socket._handle.owner.remotePort
      );
      ...
      const acceptPubkey = async (pub: string, isSync: boolean) => {
        ...
        return await this.shouldAcceptConnection(
          pub,
          isSync,
          this.getConnexionsForSync().map((c) => c.pubkey),
          this.getConnexions().map((c) => c.pubkey)
        );
      };
      let timeout = {
      ...
      const c = WS2PConnection.newConnectionFromWebSocketServer(
        ws,
        messageHandler,
        new WS2PPubkeyLocalAuth(
          this.server.conf.currency,
          key,
          myWs2pId,
          (pub) => acceptPubkey(pub, false)
        ),
      ...
      try {
        ...
        this.server.logger.info(
          "WS2P: established incoming connection from %s %s:%s",
          c.pubkey.slice(0, 8),
          host,
          port
        );
        ...
        this.server.pipe(singleWriteProtection).pipe(ws2pStreamer);
        ...
        ws.on("close", () => {
          ...
          this.server.logger.info(
            "WS2P: close incoming connection from %s %s:%s",
            c.pubkey.slice(0, 8),
            host,
            port
          );
        ...
        this.server.logger.warn(
          "WS2P: cannot connect to incoming WebSocket connection: %s",
          e
        );
      }
    });
  ...
  async removeExcessIncomingConnections() {
    ...
    const ws2pPublicMax =
      this.server.conf.ws2p && this.server.conf.ws2p.maxPublic
        ? this.server.conf.ws2p.maxPublic
        : WS2PConstants.MAX_LEVEL_2_PEERS;
    let privilegedKeys =
  ...
  async removeLowPriorityConnection(privilegedKeys: string[]) {
    ...
    let minPriorityLevel = await this.keyPriorityLevel(
      lowPriorityConnection.pubkey,
      privilegedKeys
    );
    for (const c of this.connections) {
      if (c !== lowPriorityConnection) {
        let cPriorityLevel = await this.keyPriorityLevel(
          c.pubkey,
          privilegedKeys
        );
      ...
      this.connections.map(async (c) => {
        ...
        if (c.pubkey === pubkeyOfConnection) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, WS2PConstants.CONNEXION_TIMEOUT)
          );
