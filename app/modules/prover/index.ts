...
export const ProverDependency = {
  duniter: {
    ...
    config: {
      onLoading: async (conf: ConfDTO) => {
        ...
        if (conf.nbCores === null || conf.nbCores === undefined) {
          conf.nbCores = Math.min(
            ProverConstants.CORES_MAXIMUM_USE_IN_PARALLEL,
    ...
    cliOptions: [
      {
        value: "--show",
        desc: "With gen-* commands, displays the generated block.",
      },
      {
        value: "--check",
        desc: "With gen-* commands: just check validity of generated block.",
      },
      {
        value: "--submit-local",
        desc:
      ...
      {
        value: "--at <medianTime>",
        desc: "With gen-next --show --check: allows to try in a future time.",
        parser: parseInt,
      },
    ],
        ...
        onDatabaseExecute: async (
          ...
          do {
            try {
              ...
              toDelete = await server.dal.idtyDAL.query(
                "SELECT * FROM idty i WHERE 5 > (SELECT count(*) from cert c where c.`to` = i.pubkey)"
              );
              console.log(
                "Deleting",
                toDelete.map((i: any) => i.pubkey)
              );
              await server.dal.idtyDAL.exec(
                "DELETE FROM idty WHERE pubkey IN (" +
                  toDelete.map((i: any) => "'" + i.pubkey + "'").join(",") +
                  ")"
              );
              await server.dal.idtyDAL.exec(
                "DELETE FROM cert WHERE `to` IN (" +
                  toDelete.map((i: any) => "'" + i.pubkey + "'").join(",") +
                  ")"
              );
              await server.dal.idtyDAL.exec(
                "DELETE FROM cert WHERE `from` IN (" +
                  toDelete.map((i: any) => "'" + i.pubkey + "'").join(",") +
                  ")"
              );
            }
      ...
      function (next: any) {
        ...
        (async () => {
          try {
            ...
            const trialLevel = isNaN(difficulty)
              ? await server
                  .getBcContext()
                  .getIssuerPersonalizedDifficulty(
                    server.PeeringService.selfPubkey
                  )
              : difficulty;
            const prover = new BlockProver(server);
