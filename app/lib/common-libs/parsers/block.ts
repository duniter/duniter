...
export class BlockParser extends GenericParser {
  ...
  _clean(obj: any) {
    ...
    obj.type = obj.type || "";
    obj.hash = hashf(
      rawer.getBlockInnerHashAndNonceWithSignature(obj)
    ).toUpperCase();
  ...
  _verify(obj: any) {
    ...
    if (!err) {
      ...
      if (
        !obj.version ||
        !obj.version.match(CommonConstants.DOCUMENTS_BLOCK_VERSION_REGEXP)
      )
    ...
    if (!err) {
      ...
      if (!obj.nonce || !obj.nonce.match(CommonConstants.INTEGER))
        err = {
          ...
          message: "Nonce must be an integer value",
        };
    }
    if (!err) {
      // Number
    ...
    if (!err) {
      ...
      if (!obj.medianTime || !obj.medianTime.match(CommonConstants.INTEGER))
        err = {
          code: codes.BAD_MEDIAN_TIME,
          message: "MedianTime must be an integer",
        };
    }
    if (!err) {
      if (obj.dividend && !obj.dividend.match(CommonConstants.INTEGER))
        err = {
          code: codes.BAD_DIVIDEND,
          message: "Incorrect UniversalDividend field",
        };
    }
    if (!err) {
    ...
    if (!err) {
      ...
      if (!obj.nonce || !obj.nonce.match(CommonConstants.INTEGER))
        err = {
          code: codes.BAD_MEMBERS_COUNT,
          message: "MembersCount must be an integer value",
        };
    }
    if (!err) {
      // InnerHash
      if (!obj.inner_hash || !obj.inner_hash.match(CommonConstants.FINGERPRINT))
        err = {
          code: codes.BAD_INNER_HASH,
          message: "InnerHash must be a hash value",
        };
    }
...
function splitAndMatch(separator: string, regexp: RegExp) {
  return function (raw: string) {
    ...
    lines.forEach(function (line) {
      if (line.match(regexp)) kept.push(line);
    });
    return kept;
  };
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(CommonConstants.TRANSACTION.HEADER)) {
      ...
      const linesToExtract: any = {
        ...
        signatures: {
          start:
            start + nbIssuers + nbInputs + nbUnlocks + nbOutputs + hasComment,
          end:
            start -
            1 +
            2 * nbIssuers +
      ...
      [
        ...
        for (
          let j = linesToExtract[prop].start;
          j <= linesToExtract[prop].end;
          j++
        ) {
