...
export const LOCAL_RULES_FUNCTIONS = {
  ...
  checkBlockTimes: async (block: BlockDTO, conf: ConfDTO) => {
    ...
    if (
      block.number > 0 &&
      (time < medianTime || time > medianTime + maxAcceleration(conf))
    )
      throw Error(
        "A block must have its Time between MedianTime and MedianTime + " +
          maxAcceleration(conf)
      );
  ...
  checkIdentitiesUserIDConflict: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
  ...
  checkIdentitiesPubkeyConflict: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
  ...
  checkIdentitiesMatchJoin: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
    ...
    for (const icreate of icreates) {
      ...
      if (matching.length == 0) {
        throw Error(
          "Each identity must match a newcomer line with same userid and certts"
        );
      }
    }
    return true;
  ...
  checkRevokedAreExcluded: async (
    ...
    const revocations = mindex
      .filter(
        (row: MindexEntry) =>
          !!(row.op == constants.IDX_UPDATE && row.revoked_on !== null)
      )
      .map((e) => e.pub);
    for (const pub of revocations) {
      const exclusions = Underscore.where(iindex, {
  ...
  checkCertificationOneByIssuer: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
    if (block.number > 0) {
      ...
      const certFromA = Underscore.uniq(
        cindex.map((row: CindexEntry) => row.issuer)
      );
  ...
  checkCertificationUnicity: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
    const cindex = Indexer.cindex(index);
    const certAtoB = Underscore.uniq(
      cindex.map((row: CindexEntry) => row.issuer + row.receiver)
    );
  ...
  checkCertificationIsntForLeaverOrExcluded: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
    ...
    for (const pub of certified) {
      ...
      const leavers = Underscore.where(mindex, {
        op: constants.IDX_UPDATE,
        leaving: true,
        pub: pub,
      });
      if (exclusions.length > 0 || leavers.length > 0) {
        throw Error(
          "Block cannot contain certifications concerning leavers or excluded members"
        );
      }
    }
    return true;
      ...
      for (const output of tx.outputs) {
        const out =
          typeof output === "string"
            ? output
            : TransactionDTO.outputObj2Str(output);
  ...
  checkTxSources: async (block: BlockDTO) => {
    ...
    const inputs = Underscore.filter(
      sindex,
      (row: SindexEntry) => row.op == constants.IDX_UPDATE
    ).map((row: SindexEntry) => [row.op, row.identifier, row.pos].join("-"));
    if (inputs.length !== Underscore.uniq(inputs).length) {
      throw Error(
        "It cannot exist 2 identical sources for transactions inside a given block"
      );
    }
    const outputs = Underscore.filter(
      sindex,
      (row: SindexEntry) => row.op == constants.IDX_CREATE
    ).map((row: SindexEntry) => [row.op, row.identifier, row.pos].join("-"));
  ...
  checkMaxTransactionChainingDepth: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
    ...
    //
    const allowedMax =
      block.medianTime > CommonConstants.BLOCK_TX_CHAINING_ACTIVATION_MT
        ? CommonConstants.BLOCK_MAX_TX_CHAINING_DEPTH
        : 0;
    if (max > allowedMax) {
      throw (
        "The maximum transaction chaining length per block is " +
        CommonConstants.BLOCK_MAX_TX_CHAINING_DEPTH
      );
    }
    return true;
...
function getMaxTransactionDepth(sindex: SindexShortEntry[]) {
  ...
  for (let id of ids) {
    maxTxChainingDepth = Math.max(
      maxTxChainingDepth,
      getTransactionDepth(id, sindex, 0)
    );
  }
...
function getTransactionDepth(
  txHash: string,
  sindex: SindexShortEntry[],
  localDepth = 0
) {
  const inputs = Underscore.filter(
    sindex,
    (s: SindexShortEntry) => s.op === "UPDATE" && s.tx === txHash
  );
  let depth = localDepth;
  for (let input of inputs) {
    const consumedOutput = Underscore.findWhere(sindex, {
      op: "CREATE",
    ...
    if (consumedOutput) {
      if (localDepth < 5) {
        ...
        const subTxDepth = getTransactionDepth(
          consumedOutput.tx as string,
          sindex,
          localDepth + 1
        );
...
function checkBunchOfTransactions(
...
export const LOCAL_RULES_HELPERS = {
  ...
  checkTxAmountsValidity: (tx: TransactionDTO) => {
    ...
    const commonBase: number = (inputs as BaseDTO[])
      .concat(outputs)
      .reduce((min: number, input) => {
  ...
  getMaxPossibleVersionNumber: async (
    ...
    return !current
      ? // 1. We use legacy version
        constants.BLOCK_GENESIS_VERSION
      : (async () => {
          // 2. If we can, we go to the next version
          const blocksInFrame = (
            await dal.getBlocksBetween(
              current.number - current.issuersFrame + 1,
              current.number
            )
          ).sort((b1, b2) => b2.number - b1.number);
          const uniqIssuersInFrame = Underscore.uniq(
            blocksInFrame.map((b) => b.issuer)
          );
          const lastNonceOfEachIssuer = uniqIssuersInFrame.map((issuer) =>
            String(blocksInFrame.filter((b) => b.issuer === issuer)[0].nonce)
          );
          const nbNoncesWithNextVersionCode = lastNonceOfEachIssuer.filter(
            (nonce) => nonce.substr(-11, 3) === "999"
          ).length;

          // More than 70% of the computing network converted? Let's go to next version.
          let propIssuersReadyToJump =
            nbNoncesWithNextVersionCode / uniqIssuersInFrame.length;
