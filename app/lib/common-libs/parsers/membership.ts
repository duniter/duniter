...
export class MembershipParser extends GenericParser {
  ...
  _verify(obj: any) {
    ...
    if (!err) {
      if (
        !obj.version ||
        !obj.version.match(CommonConstants.DOCUMENTS_VERSION_REGEXP)
      )
    ...
    if (!err) {
      if (!(obj.membership || "").match(/^(IN|OUT)$/))
        err = {
          code: codes.BAD_MEMBERSHIP,
          message: "Incorrect Membership field: must be either IN or OUT",
        };
    }
    if (!err) {
      if (obj.block && !obj.block.match(CommonConstants.BLOCK_UID))
        err = {
          code: codes.BAD_BLOCK,
          message:
            "Incorrect Block field: must be a positive or zero integer, a dash and an uppercased SHA1 hash",
        };
    }
    if (!err) {
      if (obj.userid && !obj.userid.match(CommonConstants.USER_ID))
        err = {
          code: codes.BAD_USERID,
          message: "UserID must match udid2 format",
        };
    }
    if (!err) {
      if (!Buid.format.isBuid(obj.certts))
        err = {
          code: codes.BAD_CERTTS,
          message: "CertTS must be a valid timestamp",
        };
    }
