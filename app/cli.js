...
module.exports = (programArgs) => {
  return new DuniterCommand(programArgs);
};

function DuniterCommand(programArgs) {

  let that = this;

  // Default close process: does nothing
  this.closeCommand = () => {};

  this.execute = () => co(function*() {
    ...
    function subCommand(promiseFunc) {
      return function() {
        ...
        return co(function*() {
          try {
            yield promiseFunc.apply(null, args);
            onResolve();
          } catch (e) {
            logger.error(e);
            onReject();
