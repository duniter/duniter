...
export const ALIAS = {
  ALL_LOCAL: async (block: BlockDTO, conf: ConfDTO, index: IndexEntry[]) => {
    ...
    await LOCAL_RULES_FUNCTIONS.checkIdentitiesUserIDConflict(
      block,
      conf,
      index
    );
    await LOCAL_RULES_FUNCTIONS.checkIdentitiesPubkeyConflict(
      block,
      conf,
      index
    );
    ...
    await LOCAL_RULES_FUNCTIONS.checkCertificationOneByIssuer(
      block,
      conf,
      index
    );
    await LOCAL_RULES_FUNCTIONS.checkCertificationUnicity(block, conf, index);
    await LOCAL_RULES_FUNCTIONS.checkCertificationIsntForLeaverOrExcluded(
      block,
      conf,
      index
    );
    ...
    await LOCAL_RULES_FUNCTIONS.checkMaxTransactionChainingDepth(
      block,
      conf,
      index
    );
  ...
  ALL_LOCAL_BUT_POW_AND_SIGNATURE: async (
    block: BlockDTO,
    conf: ConfDTO,
    index: IndexEntry[]
  ) => {
