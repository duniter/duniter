...
export function evalParams(
  ...
  for (const func of params) {
    if (func.match(/^SIG/)) {
      ...
      const sigEntry =
        !isNaN(index) && index < issuers.length && sigResult.sigs[index];
      const signatory: { k: string; ok: boolean } = sigEntry || {
        k: "",
        ok: false,
      };
      res.push({
...
export const GLOBAL_RULES_FUNCTIONS = {
  checkIdentitiesAreWritable: async (
    block: { identities: string[]; version: number },
    conf: ConfDTO,
    dal: FileDAL
  ) => {
  ...
  checkSourcesAvailability: async (
    ...
    conf: ConfDTO,
    dal: FileDAL,
    findSourceTx: (txHash: string) => Promise<DBTx | null>
  ) => {
...
export const GLOBAL_RULES_HELPERS = {
  ...
  checkMembershipBlock: (
    ...
    checkMSTarget(
      ms,
      current ? { number: current.number + 1 } : { number: 0 },
      conf,
      dal
    ),

  checkCertificationIsValidInSandbox: (
    cert: any,
    current: BlockDTO,
    findIdtyFunc: any,
    conf: ConfDTO,
    ...
    return checkCertificationShouldBeValid(
      current ? current : { number: 0, currency: "" },
      cert,
      findIdtyFunc,
      conf,
      dal
  ...
  checkCertificationIsValidForBlock: (
    cert: any,
    block: { number: number; currency: string },
    findIdtyFunc: (
      b: { number: number; currency: string },
      pubkey: string,
      dal: FileDAL
    ) => Promise<{
      pubkey: string;
  ...
  isOver3Hops: async (
    ...
    try {
      return Indexer.DUP_HELPERS.checkPeopleAreNotOudistanced(
        [member],
        newLinks,
        newcomers,
        conf,
  ...
  checkExistsUserID: (uid: string, dal: FileDAL) =>
    dal.getWrittenIdtyByUIDForExistence(uid),

  checkExistsPubkey: (pub: string, dal: FileDAL) =>
    dal.getWrittenIdtyByPubkeyForExistence(pub),
  ...
  checkSingleTransaction: (
    ...
    GLOBAL_RULES_FUNCTIONS.checkSourcesAvailability(
      {
  ...
  checkTxBlockStamp: async (tx: TransactionDTO, dal: FileDAL) => {
    ...
    if (
      current &&
      current.medianTime > basedBlock.medianTime + constants.TX_WINDOW
    ) {
...
async function checkMSTarget(ms: any, block: any, conf: ConfDTO, dal: FileDAL) {
  if (block.number == 0 && ms.number != 0) {
    ...
    block.number == 0 &&
    ms.fpr != "E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855"
  ) {
    throw Error(
      "Hash must be E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855 for root block's memberships"
    );
    ...
    if (
      current &&
      current.medianTime > basedBlock.medianTime + conf.msValidity
    ) {
...
async function checkCertificationShouldBeValid(
  block: { number: number; currency: string },
  cert: any,
  findIdtyFunc: (
    b: { number: number; currency: string },
  ...
  if (block.number == 0 && cert.block_number != 0) {
    ...
    if (!idty) {
      ...
      if (
        cert.block_hash &&
        buid != [cert.block_number, cert.block_hash].join("-")
      )
        throw Error(
          "Certification based on an unexisting block buid. from " +
            cert.from.substring(0, 8) +
            " to " +
