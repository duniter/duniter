...
export class BlockchainService extends FIFOService {
  ...
  constructor(private server: Server, fifoPromiseHandler: GlobalFifoPromise) {
    ...
    this.switcherDao = new (class ForkDao implements SwitcherDao<BlockDTO> {
      ...
      async getPotentials(
        ...
        const blocks = await this.bcService.dal.getPotentialForkBlocks(
          numberStart,
          timeStart,
          maxNumber
        );
      ...
      async getBlockchainBlock(
        ...
        const b = await this.bcService.dal.getAbsoluteValidBlockInForkWindow(
          number,
          hash
        );
        if (!b) return null;
      ...
      async getAbsoluteBlockInForkWindow(
        ...
        const block = await this.bcService.dal.getAbsoluteBlockInForkWindow(
          number,
          hash
        );
        if (block) {
  ...
  submitBlock(blockToAdd: any, noResolution = false): Promise<BlockDTO> {
    const obj = parsers.parseBlock.syncWrite(
      BlockDTO.fromJSONObject(blockToAdd).getRawSigned()
    );
    ...
    return this.pushFIFO(hash, async () => {
      ...
      if (
        this.conf &&
        this.conf.currency &&
        this.conf.currency !== dto.currency
      ) {
      ...
      const absolute = await this.dal.existsAbsoluteBlockInForkWindow(
        parseInt(obj.number),
        obj.hash
      );
      if (!absolute) {
  ...
  async blockResolution(
    filterFunc: (b: DBBlock) => boolean = () => true
  ): Promise<BlockDTO | null> {
    ...
    do {
      ...
      let potentials = [];
      if (current) {
        potentials = (await this.dal.getForkBlocksFollowing(current)).filter(
          filterFunc
        );
        this.logger.info(
          "Block resolution: %s potential blocks after current#%s...",
          potentials.length,
          current.number
        );
      } else {
        potentials = (await this.dal.getPotentialRootBlocks()).filter(
          filterFunc
        );
        this.logger.info(
          "Block resolution: %s potential blocks for root block...",
          potentials.length
        );
      }
      added = null;
  ...
  revertCurrentBlock() {
    return this.pushFIFO("revertCurrentBlock", () =>
      this.mainContext.revertCurrentBlock()
    );
  }

  revertCurrentHead() {
    return this.pushFIFO("revertCurrentHead", () =>
      this.mainContext.revertCurrentHead()
    );
  }

  applyNextAvailableFork() {
    return this.pushFIFO("applyNextAvailableFork", () =>
      this.mainContext.applyNextAvailableFork()
    );
  }

  async requirementsOfIdentities(
    identities: IdentityForRequirements[],
    ...
    for (const obj of identities) {
      try {
        let reqs = await this.requirementsOfIdentity(
          obj,
          current,
          computeDistance
        );
  ...
  async requirementsOfIdentity(
    idty: IdentityForRequirements,
    current: DBBlock | null,
    computeDistance = true
  ): Promise<HttpIdentityRequirement> {
    ...
    let mssPending = [];
    try {
      const join = await this.server.generatorGetJoinData(
        current,
        idty.hash,
        "a"
      );
      ...
      // Check WoT stability
      const someNewcomers = join.identity.wasMember
        ? []
        : [join.identity.pubkey];
      ...
      const newCerts = await this.server.generatorComputeNewCerts(
        nextBlockNumber,
        [join.identity.pubkey],
        joinData,
        updates
      );
      const newLinks = await this.server.generatorNewCertsToLinks(
        newCerts,
        updates
      );
      ...
      if (computeDistance) {
        outdistanced = await GLOBAL_RULES_HELPERS.isOver3Hops(
          pubkey,
          newLinks,
          someNewcomers,
          current,
      ...
      if (currentMSN >= 0) {
        if (join.identity.member) {
          ...
          if (msBlock) {
            ...
            expiresMS = Math.max(
              0,
              msBlock.medianTime + this.conf.msValidity - currentTime
            );
          } else {
      ...
      isSentry =
        idty.member && (await this.dal.isSentry(idty.pubkey, this.conf));
    } catch (e) {
      ...
      if (
        !(
          e &&
          e.uerr &&
          e.uerr.ucode == constants.ERRORS.TOO_OLD_IDENTITY.uerr.ucode
        )
      ) {
        throw e;
      } else {
  ...
  async getValidCerts(
    ...
    const certsFromLinks = links.map((lnk: any) => {
      return {
        from: lnk.issuer,
        to: lnk.receiver,
        sig: lnk.sig,
    ...
    return certsFromLinks.concat(certsFromCerts).map((c) => {
      c.expiresIn = Math.max(
        0,
        c.timestamp + this.conf.sigValidity - currentTime
      );
      return c;
    });
  }
