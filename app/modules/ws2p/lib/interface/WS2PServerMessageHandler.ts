...
export class WS2PServerMessageHandler implements WS2PMessageHandler {
    ...
    try {
      if (json.body) {
        if (json.body.block) {
          ...
          json.body.heads &&
          typeof json.body.heads === "object" &&
          json.body.heads.length !== undefined
        ) {
      ...
      if (
        ...
        this.server.conf.pair.pub !== c.pubkey
      ) {
        // We do not want to ban ourselves
        this.errors[documentHash].pubkeys[c.pubkey].push(json.body);
        if (
          this.errors[documentHash].pubkeys[c.pubkey].length >=
          WS2PConstants.BAN_ON_REPEAT_THRESHOLD
        ) {
          let message =
            "peer " +
            (c.pubkey || "--unknown--") +
            " sent " +
            WS2PConstants.BAN_ON_REPEAT_THRESHOLD +
            " times a same wrong document: " +
            ((e && (e.message || (e.uerr && e.uerr.message))) ||
              JSON.stringify(e));
      ...
      if (
        ...
        (!e.uerr ||
          !(
            ...
            e.uerr.ucode ==
              CommonConstants.ERRORS.PEER_DOCUMENT_ALREADY_KNOWN.uerr.ucode
          ))
      ) {
  ...
  async answerToRequest(data: any, c: WS2PConnection): Promise<WS2PResponse> {
    ...
    if (
      ...
      (!data ||
        !data.name ||
        forbiddenRequestsForSync.indexOf(data.name) !== -1)
    ) {
