...
export class MembershipDAL extends AbstractSQLite<DBMembership> {
  ...
  getSandboxMemberships() {
    return this.query(
      "SELECT * FROM sandbox_memberships LIMIT " + this.sandbox.maxSize,
      []
    );
  }

  sandbox = new SandBox(
    constants.SANDBOX_SIZE_MEMBERSHIPS,
    this.getSandboxMemberships.bind(this),
    (
      compared: { block_number: number; issuers: string[] },
      reference: { block_number: number; issuers: string[] }
    ) => {
